#добавление новых модулей (неудачно)
...
class Application(tk.Tk):
    ...
    def create_widgets(self):
        tk.Label(self, text="Логин:").grid(row=0, column=0, sticky=tk.W)
        self.entry_username = tk.Entry(self)
        self.entry_username.grid(row=0, column=1, sticky=(tk.W, tk.E))

        tk.Label(self, text="Пароль:").grid(row=1, column=0, sticky=tk.W)
        self.entry_password = tk.Entry(self, show="*")
        self.entry_password.grid(row=1, column=1, sticky=(tk.W, tk.E))

        tk.Button(self, text="Вход", command=self.login).grid(row=2, column=0, columnspan=2, pady=5)
...
class MainWindow(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.current_time_label = tk.Label(self, text="", font=("Helvetica", 12))
        self.current_time_label.grid(row=0, column=0, columnspan=3, pady=5)

        tk.Label(self, text="Название проекта:").grid(row=1, column=0, sticky=tk.W)
        self.entry_project_name = tk.Entry(self)
        self.entry_project_name.grid(row=1, column=1, columnspan=2, sticky=(tk.W, tk.E))

        tk.Label(self, text="Тип проекта:").grid(row=2, column=0, sticky=tk.W)
        self.entry_project_type = tk.Entry(self)
        self.entry_project_type.grid(row=2, column=1, columnspan=2, sticky=(tk.W, tk.E))

        tk.Label(self, text="Описание проекта:").grid(row=3, column=0, sticky=tk.W)
        self.entry_project_discription = tk.Entry(self)
        self.entry_project_discription.grid(row=3, column=1, columnspan=2, sticky=(tk.W, tk.E))

        tk.Label(self, text="Дата начала:").grid(row=4, column=0, sticky=tk.W)
        self.entry_start_date = tk.Entry(self)
        self.entry_start_date.grid(row=4, column=1, columnspan=2, sticky=(tk.W, tk.E))

        tk.Label(self, text="Дата окончания:").grid(row=5, column=0, sticky=tk.W)
        self.entry_end_date = tk.Entry(self)
        self.entry_end_date.grid(row=5, column=1, columnspan=2, sticky=(tk.W, tk.E))

        tk.Label(self, text="Прикрепленный файл:").grid(row=6, column=0, sticky=tk.W)
        self.entry_file_path = tk.Entry(self)
        self.entry_file_path.grid(row=6, column=1, sticky=(tk.W, tk.E))
        tk.Button(self, text="Выбрать файл", command=self.select_file).grid(row=6, column=2, sticky=(tk.W, tk.E))

        tk.Button(self, text="Добавить проект", command=self.add_project).grid(row=7, column=0, columnspan=3, pady=5)
        tk.Button(self, text="Выход", command=self.logout).grid(row=8, column=0, columnspan=3, pady=5)

        self.project_frame = tk.Frame(self)
        self.project_frame.grid(row=9, column=0, columnspan=3, sticky=(tk.W, tk.E, tk.N, tk.S))
    ...
    def add_project(self):
        ...
        project_discription = self.entry_project_discription.get()
        start_date = self.entry_start_date.get()
        end_date = self.entry_end_date.get()
        file_path = self.entry_file_path.get()
        self.db_manager.execute_query(
            "INSERT INTO projects (name, type, description, start_date, end_date, file_path, user_id) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (project_name, project_type, project_discription, start_date, end_date, file_path, self.user_id)
        )
        self.display_projects()
    ...
    def display_projects(self):
        ...
        for project in projects:
            ...
            label.pack(anchor='w')
            if project[6]:
                file_label = tk.Label(self.project_frame, text=f"Файл: {project[6]}")
    ...
    def show_users(self):
        ...
        for user in users:
            user_label = tk.Label(users_window, text=f"{user[0]}: {user[1]}")
