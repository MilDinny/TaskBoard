#Проекты выведены в отдельное окно при создании нового
...
class Application(tk.Tk):
    ...
    def show_users(self):
        ...
        for user in users:
            user_label = tk.Label(users_window, text=f"Пользователь: {user[1]}", font=("Helvetica", 12))
...
class MainWindow(tk.Tk):
    ...
    def create_widgets(self):
        ...
        tk.Button(self, text="Проекты", command=self.open_projects_window, font=("Helvetica", 14)).grid(row=1, column=0,
                                                                                                        columnspan=3,
                                                                                                        pady=10)
        tk.Button(self, text="Список пользователей", command=self.show_users, font=("Helvetica", 14)).grid(row=2,
                                                                                                           column=0,
                                                                                                           columnspan=3,
                                                                                                           pady=10)
        tk.Button(self, text="Удалить свой аккаунт", command=self.delete_own_account, font=("Helvetica", 14)).grid(
            row=3, column=0, columnspan=3, pady=10)
        tk.Button(self, text="Выход", command=self.logout, font=("Helvetica", 14)).grid(row=4, column=0, columnspan=3,
                                                                                        pady=10)
    ...
    def open_projects_window(self):
        ProjectsWindow(self.db_manager, self.user_id)
...
class ProjectsWindow(tk.Toplevel):
    ...
    def create_widgets(self):
        tk.Label(self, text="Название проекта:", font=("Helvetica", 14)).grid(row=0, column=0, sticky=tk.W, padx=10,
                                                                              pady=10)
        self.entry_project_name = tk.Entry(self, font=("Helvetica", 14))
        self.entry_project_name.grid(row=0, column=1, columnspan=2, sticky=(tk.W, tk.E), padx=10, pady=10)

        tk.Label(self, text="Тип проекта:", font=("Helvetica", 14)).grid(row=1, column=0, sticky=tk.W, padx=10, pady=10)
        self.entry_project_type = tk.Entry(self, font=("Helvetica", 14))
        self.entry_project_type.grid(row=1, column=1, columnspan=2, sticky=(tk.W, tk.E), padx=10, pady=10)

        tk.Label(self, text="Дата начала:", font=("Helvetica", 14)).grid(row=2, column=0, sticky=tk.W, padx=10, pady=10)
        self.entry_start_date = tk.Entry(self, font=("Helvetica", 14))
        self.entry_start_date.grid(row=2, column=1, columnspan=2, sticky=(tk.W, tk.E), padx=10, pady=10)

        tk.Label(self, text="Дата окончания:", font=("Helvetica", 14)).grid(row=3, column=0, sticky=tk.W, padx=10,
                                                                            pady=10)
        self.entry_end_date = tk.Entry(self, font=("Helvetica", 14))
        self.entry_end_date.grid(row=3, column=1, columnspan=2, sticky=(tk.W, tk.E), padx=10, pady=10)

        tk.Label(self, text="Прикрепленный файл:", font=("Helvetica", 14)).grid(row=4, column=0, sticky=tk.W, padx=10,
                                                                                pady=10)
        self.entry_file_path = tk.Entry(self, font=("Helvetica", 14))
        self.entry_file_path.grid(row=4, column=1, sticky=(tk.W, tk.E), padx=10, pady=10)
        tk.Button(self, text="Выбрать файл", command=self.select_file, font=("Helvetica", 14)).grid(row=4, column=2,
                                                                                                    sticky=(tk.W, tk.E),
                                                                                                    padx=10, pady=10)

        tk.Button(self, text="Добавить проект", command=self.add_project, font=("Helvetica", 14)).grid(row=5, column=0,
                                                                                                       columnspan=3,
                                                                                                       pady=10)

        self.project_frame = tk.Frame(self)
        self.project_frame.grid(row=6, column=0, columnspan=3, sticky=(tk.W, tk.E, tk.N, tk.S), padx=10, pady=10)
