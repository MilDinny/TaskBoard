#Разработка телеграмм бота для приложения (неудачно на данный момент)
...
class MainWindow(tk.Tk):
    ...
    def create_widgets(self):
        tk.Label(self, text="Название проекта:").grid(row=0, column=0, sticky=tk.W)
        self.entry_project_name = tk.Entry(self)
        self.entry_project_name.grid(row=0, column=1, sticky=(tk.W, tk.E))

        tk.Label(self, text="Тип проекта:").grid(row=1, column=0, sticky=tk.W)
        self.entry_project_type = tk.Entry(self)
        self.entry_project_type.grid(row=1, column=1, sticky=(tk.W, tk.E))

        tk.Label(self, text="Дата начала:").grid(row=2, column=0, sticky=tk.W)
        self.entry_start_date = tk.Entry(self)
        self.entry_start_date.grid(row=2, column=1, sticky=(tk.W, tk.E))

        tk.Label(self, text="Дата окончания:").grid(row=3, column=0, sticky=tk.W)
        self.entry_end_date = tk.Entry(self)
        self.entry_end_date.grid(row=3, column=1, sticky=(tk.W, tk.E))

        tk.Label(self, text="Прикрепленный файл:").grid(row=4, column=0, sticky=tk.W)
        self.entry_file_path = tk.Entry(self)
        self.entry_file_path.grid(row=4, column=1, sticky=(tk.W, tk.E))
        tk.Button(self, text="Выбрать файл", command=self.select_file).grid(row=4, column=2, sticky=(tk.W, tk.E))

        tk.Button(self, text="Добавить проект", command=self.add_project).grid(row=5, column=0, columnspan=3, pady=5)
                ...
                if now.year == year and now.month == month:
                    if now.day > day - DAYS_BEFORE:
                        self.send_telegram_message(
                            f"До сдачи проекта {project_name} осталось менее {DAYS_BEFORE} суток")
                        messagebox.showerror("Важно", "До сдачи проекта " + project_name + " осталось менее " + str(
                            DAYS_BEFORE) + " суток")
                    elif now.day == day:
                        self.send_telegram_message(f"Сегодня день сдачи проекта {project_name}")
                        messagebox.showerror("Важно", "Сегодня день сдачи проекта " + project_name)
                    elif now.day > day:
                        self.send_telegram_message(f"Проект {project_name} просрочен!")
