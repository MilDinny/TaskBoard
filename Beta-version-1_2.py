...
class Application(tk.Tk):
    ...
    def create_widgets(self):
        ...
        tk.Label(frame, text="Подтвердите пароль:", font=("Helvetica", 14)).grid(row=3, column=0, sticky=tk.E, padx=10, pady=10)
        self.entry_confirm_password = tk.Entry(frame, show="*", font=("Helvetica", 14))
        self.entry_confirm_password.grid(row=3, column=1, sticky=(tk.W, tk.E), padx=10, pady=10)

        tk.Button(frame, text="Вход", command=self.login, font=("Helvetica", 14)).grid(row=4, column=0, columnspan=2, pady=10)
        tk.Button(frame, text="Регистрация", command=self.register, font=("Helvetica", 14)).grid(row=5, column=0, columnspan=2, pady=10)
        tk.Button(frame, text="Список пользователей", command=self.show_users, font=("Helvetica", 14)).grid(row=6, column=0, columnspan=2, pady=10)
        tk.Button(frame, text="Выход", command=self.quit_application, font=("Helvetica", 14)).grid(row=7, column=0, columnspan=2, pady=10)
    ...
    def login(self):
        ...
        if user_id:
            self.destroy()
            MainWindow(self.db_manager, user_id, username)
...
class MainWindow(tk.Tk):
    ...
    def create_widgets(self):
        ...
        tk.Button(frame, text="Выход", command=self.logout, font=("Helvetica", 14)).grid(row=5, column=0, columnspan=2, pady=10)
        ...
        for project in projects:
            ...
            try:
                ...
                if not project[5]:
                    ...
                    elif now.year > year or (now.year == year and теперь +year+ 'не совпадают'):
                        messagebox.showerror("Важно", "Проект " + project_name + " просрочен!")
