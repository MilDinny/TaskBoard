#кнопка выхода в окне логина/регистрации
...
class Application(tk.Tk):
    ...
    def create_widgets(self):
        ...
        tk.Button(self, text="Выход", command=self.quit_application, font=("Helvetica", 14)).grid(row=5, column=0, columnspan=2, pady=10)
...
class MainWindow(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.greeting_label = tk.Label(self, text=greeting, font=("Helvetica", 16))
        self.greeting_label.grid(row=0, column=0, columnspan=3, pady=10)

        self.current_time_label = tk.Label(self, text="", font=("Helvetica", 14))
        self.current_time_label.grid(row=1, column=0, columnspan=3, pady=10)

        tk.Button(self, text="Проекты", command=self.open_projects_window, font=("Helvetica", 14)).grid(row=2, column=0, columnspan=3, pady=10)
        tk.Button(self, text="Удалить свой аккаунт", command=self.delete_own_account, font=("Helvetica", 14)).grid(row=3, column=0, columnspan=3, pady=10)
        tk.Button(self, text="Выход", command=self.logout, font=("Helvetica", 14)).grid(row=4, column=0, columnspan=3, pady=10)
