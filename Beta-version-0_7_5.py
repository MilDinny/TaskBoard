#Список пользователей перенесен в окно входа и регистрации и теперь отображает пользователей корректно
...
class ProjectsWindow(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        tk.Button(self, text="Добавить проект", command=self.add_project, font=("Helvetica", 14)).grid(row=6, column=0, columnspan=3, pady=10)
        ...
        self.project_frame.bind("<Configure>", lambda event: self.canvas.configure(scrollregion=self.canvas.bbox("all")))

        self.display_projects()

        tk.Button(self, text="Выход", command=self.close_window, font=("Helvetica", 14)).grid(row=8, column=0, columnspan=3, pady=10)
