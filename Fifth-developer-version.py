#добавлена возможность удалить проекты
#разрабатывается возможность удалять файлы и пользователей
#разрабатывается меню существующих пользователей
...
class MainWindow(tk.Tk):
    ...
    def create_widgets(self):
        ...
        user_menu.add_command(label="Удалить пользователя", command=self.delete_user)
        ...
        tk.Label(self, text="Дата начала:").grid(row=3, column=0, sticky=tk.W)
        self.entry_start_date = tk.Entry(self)
        self.entry_start_date.grid(row=3, column=1, columnspan=2, sticky=(tk.W, tk.E))

        tk.Label(self, text="Дата окончания:").grid(row=4, column=0, sticky=tk.W)
        self.entry_end_date = tk.Entry(self)
        self.entry_end_date.grid(row=4, column=1, columnspan=2, sticky=(tk.W, tk.E))

        tk.Label(self, text="Прикрепленный файл:").grid(row=5, column=0, sticky=tk.W)
        self.entry_file_path = tk.Entry(self)
        self.entry_file_path.grid(row=5, column=1, sticky=(tk.W, tk.E))
        tk.Button(self, text="Выбрать файл", command=self.select_file).grid(row=5, column=2, sticky=(tk.W, tk.E))

        tk.Button(self, text="Добавить проект", command=self.add_project).grid(row=6, column=0, columnspan=3, pady=5)
        tk.Button(self, text="Выход", command=self.logout).grid(row=7, column=0, columnspan=3, pady=5)

        self.project_frame = tk.Frame(self)
        self.project_frame.grid(row=8, column=0, columnspan=3, sticky=(tk.W, tk.E, tk.N, tk.S))
    ...
    def delete_user(self):
        user_id = tk.simpledialog.askinteger("Удалить пользователя", "Введите ID пользователя для удаления:")
