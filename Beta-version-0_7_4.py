#Добавлено пролистывание имеющихся проектов (но отображение пока работает не корректно)

import tkinter as tk
from tkinter import messagebox, filedialog, Menu, Scrollbar, Frame, Canvas
import sqlite3
import hashlib
import datetime
...
class ProjectsWindow(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        # Создание области прокрутки для списка проектов
        self.canvas = Canvas(self)
        self.canvas.grid(row=7, column=0, columnspan=3, sticky=(tk.W, tk.E, tk.N, tk.S))
        self.scrollbar = Scrollbar(self, orient="vertical", command=self.canvas.yview)
        self.scrollbar.grid(row=7, column=3, sticky=(tk.N, tk.S))
        self.canvas.configure(yscrollcommand=self.scrollbar.set)

        self.project_frame = Frame(self.canvas)
        self.canvas.create_window((0, 0), window=self.project_frame, anchor='nw')

        self.project_frame.bind("<Configure>",
    ...
    def display_projects(self):
        ...
        for project in projects:
            project_frame = Frame(self.project_frame, padx=10, pady=5)
